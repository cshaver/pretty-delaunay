(function() {
  ...
  const formWrapperWrapper = document.getElementById('formWrapperWrapper');
  ...
  formWrapperWrapper.addEventListener('mousemove', (e) => {
    showForm();
  }, false);

  formWrapperWrapper.addEventListener('mouseout', (e) => {
    hideForm();
  }, false);
  ...
  function runDelaunay() {
    ...
    setColors(prettyDelaunay.getColors());
  }

  function hideForm() {
    formWrapper.className = 'hidden';
  }
  ...
  function setColors(colors) {
    ...
    for (var i = 0; i < colors.length; i++) {
      var hex = Color.hslToHex(colors[i]);
      document.getElementById('color' + (i + 1)).value = '#' + hex;
      console.log('%c' + colors[i], 'color:' + consoleColor + ';background:#' + hex);
    }
  }
